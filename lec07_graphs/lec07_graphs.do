...
cd "da-coding-stata/lec07_graphs/"
...
graph export "figure_7_5.png", as(png) replace width(3200) height(2400)	
...
*** 1) There are some pre-defined schemes in Stata that can be used to easliy format our graphs...
*** ... For example, let us make a scatter in the style of The Economist graphs:
use "hotels-vienna.dta", clear
graph twoway (scatter price distance) (lfit price distance), scheme(economist)
*** The list of pre-installed schemes is available using the following command:
graph query, schemes

*** 2) Stata graphs (.gph files) can be edited interactively (i.e. using the clicking method) using the Graph Editor. 
